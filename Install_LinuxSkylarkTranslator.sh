...
cleanup_old_temp() {
    ...
    for pattern in "${system_tmp_patterns[@]}"; do
        if ls $pattern 2>/dev/null | head -1 >/dev/null; then
            rm -rf $pattern 2>/dev/null && log "已清理系统临时文件: $pattern"
        fi
    done
    
    # 特别处理 skylark_build_temp 目录（精确匹配）
    local skylark_temp_paths=(
        "$HOME/skylark_build_temp"
        "$HOME/skylark_build_temp-*"
        "$SCRIPT_DIR/skylark_build_temp"
        "$SCRIPT_DIR/skylark_build_temp-*"
    )
    
    for temp_path in "${skylark_temp_paths[@]}"; do
        # 使用 find 命令确保找到所有变体
        if [[ "$temp_path" == *"*"* ]]; then
            # 处理带通配符的路径
            for found_path in $temp_path; do
                if [[ -e "$found_path" ]]; then
                    rm -rf "$found_path" 2>/dev/null && log "已清理: $found_path"
                fi
            done
        else
            # 处理精确路径
            if [[ -e "$temp_path" ]]; then
                rm -rf "$temp_path" 2>/dev/null && log "已清理: $temp_path"
            fi
        fi
    done
    
    # 使用 find 命令深度搜索并删除所有 skylark 相关临时目录
    log "深度搜索 skylark 临时文件..."
    while IFS= read -r -d '' temp_dir; do
        if [[ -d "$temp_dir" ]]; then
            rm -rf "$temp_dir" 2>/dev/null && log "深度清理: $temp_dir"
        fi
    done < <(find /tmp "$HOME" "$SCRIPT_DIR" -maxdepth 3 -type d -name "*skylark*build*temp*" -print0 2>/dev/null)
    
    # 显示清理结果
    if [[ -d "$HOME/skylark_build_temp" ]]; then
        warn "警告: $HOME/skylark_build_temp 仍然存在，尝试强制删除..."
        rm -rf "$HOME/skylark_build_temp" 2>/dev/null && log "强制删除成功" || error "强制删除失败"
    fi
...
# 新增：专门针对 skylark_build_temp 的清理函数
cleanup_skylark_build_temp() {
    log "专门清理 skylark_build_temp 目录..."
    ...
    # 添加可能的变体
    target_paths+=($(find /tmp "$HOME" "$SCRIPT_DIR" -maxdepth 2 -type d -name "skylark_build_temp*" 2>/dev/null))
    ...
    for path in "${target_paths[@]}"; do
        if [[ -n "$path" && -e "$path" ]]; then
            ...
            if rm -rf "$path" 2>/dev/null; then
                ...
                # 如果普通删除失败，尝试使用 sudo（如果需要）
                warn "普通删除失败，尝试其他方法..."
                if [[ -d "$path" ]]; then
                    # 尝试更改权限后删除
                    chmod -R 755 "$path" 2>/dev/null
                    rm -rf "$path" 2>/dev/null && success "通过权限更改后删除成功" || error "删除失败: $path"
                fi
            fi
        fi
    done
    ...
    # 最终确认
    if [[ ! -d "$HOME/skylark_build_temp" ]]; then
        success "skylark_build_temp 目录已彻底清理"
    else
        error "skylark_build_temp 目录仍然存在，可能需要手动删除"
        ls -la "$HOME/skylark_build_temp" 2>/dev/null | head -10 | while read line; do warn "目录内容: $line"; done
    fi
}
# 主函数
main() {
