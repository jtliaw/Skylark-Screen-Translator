#!/bin/bash
# Skylark Screen Translator - Linux 通用启动器
...
verify_installation() {
    ...
    if command -v tesseract &> /dev/null; then
        ...
        log "✓ Tesseract OCR: $tesseract_version"
...
#!/bin/bash
# Skylark Screen Translator - 直接启动脚本
# 此脚本直接启动应用，无需重复检查环境
...
echo "正在启动 $APP_NAME..."
if python3 -c "import argostranslate" 2>/dev/null; then
    echo "✓ argostranslate 可用"
else
    echo "△ argostranslate 不可用，将在仅 OCR 模式下启动"
fi
...
Path=$SCRIPT_DIR"
    ...
    success "已创建快速启动脚本: $direct_launcher"
    success "桌面图标已创建: $desktop_file_path"
    info "双击桌面图标将直接启动应用（秒级启动）"
    info "如需重新配置环境，请运行: $SCRIPT_DIR/$(basename "$0")"
}
...
launch_script() {
    ...
    # 确保 argos-translate 目录结构正确（额外保险）
    local argos_packages_dir="$HOME/.local/share/argos-translate/packages"
    if [[ -e "$argos_packages_dir" ]] && [[ ! -d "$argos_packages_dir" ]]; then
        warn "检测到损坏的 packages 路径，正在修复..."
        rm -rf "$argos_packages_dir"
        mkdir -p "$argos_packages_dir"
        success "已修复 packages 目录"
    fi
...
cleanup_installation_temp() {
    ...
    # 清理 skylark_build_temp 文件夹
    local build_temp_dir="$HOME/skylark_build_temp"
    if [[ -d "$build_temp_dir" ]]; then
        local build_temp_size=$(du -sh "$build_temp_dir" 2>/dev/null | cut -f1)
        
        echo
        read -p "检测到构建临时文件夹 skylark_build_temp (大小: $build_temp_size)，是否删除？ (y/N): " -n 1 -r
        echo
        
        if [[ $REPLY =~ ^[Yy]$ ]]; then
            rm -rf "$build_temp_dir"
            success "已删除构建临时文件夹: $build_temp_dir"
        else
            info "保留构建临时文件夹: $build_temp_dir"
        fi
    fi
}
...
show_completion_info() {
    ...
    echo -e "  1. 双击桌面图标启动"
    echo -e "  2. 或运行: ${BLUE}$(basename "$0")${NC}"
    echo -e "  3. 或在应用程序菜单中查找"
    echo
    ...
    echo
    echo -e "${BLUE}如需安装翻译功能：${NC}"
    echo -e "  cd $SCRIPT_DIR && source venv/bin/activate && pip install argostranslate"
    echo
    echo -e "${BLUE}目录结构：${NC}"
    echo -e "  $SCRIPT_DIR/"
    ...
    echo -e "  ├── tmp/                    # 临时下载文件"
    echo -e "  ├── skylark_direct.sh       # 快速启动脚本"
...
# 检查并移除可能存在的 argos-translate packages 连接
cleanup_argos_packages_links() {
    local argos_packages_dir="$HOME/.local/share/argos-translate"
    
    # 如果存在整个 argos-translate 目录，则删除整个目录
    if [[ -e "$argos_packages_dir" ]]; then
        info "检测到 argos-translate 目录: $argos_packages_dir"
        
        # 获取目录大小
        local dir_size=$(du -sh "$argos_packages_dir" 2>/dev/null | cut -f1 || echo "unknown")
        warn "将删除整个 argos-translate 目录 (大小: $dir_size)"
        
        # 询问用户确认
        read -p "是否继续删除 argos-translate 目录？ (Y/n): " -n 1 -r
        echo
        if [[ ! $REPLY =~ ^[Nn]$ ]]; then
            # 强制删除整个目录
            rm -rf "$argos_packages_dir"
            ...
            # 验证是否删除成功
            if [[ -e "$argos_packages_dir" ]]; then
                error "无法删除 argos-translate 目录，可能存在权限问题"
                return 1
            else
                success "已成功删除 argos-translate 目录"
            fi
        else
            warn "用户选择保留 argos-translate 目录，可能会遇到安装问题"
            return 0
        fi
    else
        log "未找到 argos-translate 目录，无需清理"
    fi
    
    # 重新创建目录结构
    log "重新创建空目录: $argos_packages_dir"
    mkdir -p "$argos_packages_dir/packages"
    
    # 验证目录是否创建成功
    if [[ -d "$argos_packages_dir/packages" ]]; then
        success "已成功创建空目录: $argos_packages_dir/packages"
    else
        error "无法创建目录: $argos_packages_dir/packages"
        return 1
    fi
    
    # 额外检查：清理可能存在的其他冲突目录
    local potential_conflicts=(
        "$HOME/.local/share/argos_translate"
    ...
    for conflict_dir in "${potential_conflicts[@]}"; do
        if [[ -e "$conflict_dir" ]]; then
            warn "发现可能的冲突路径: $conflict_dir"
            local size=$(du -sh "$conflict_dir" 2>/dev/null | cut -f1 || echo "unknown")
            read -p "是否删除冲突路径 $conflict_dir (大小: $size)？ (y/N): " -n 1 -r
            echo
            if [[ $REPLY =~ ^[Yy]$ ]]; then
                rm -rf "$conflict_dir"
                log "已删除冲突路径: $conflict_dir"
            else
                info "保留冲突路径: $conflict_dir"
            fi
        fi
    done
}
...
main() {
    ...
    # 清理可能存在的 argos-translate packages 连接（新增）
    info "检查并清理可能存在的 argos-translate packages 连接..."
    cleanup_argos_packages_links
