...
# 确保 argospm 在 PATH 中（无论在 usr/bin 还是 _internal/bin）
export PATH="${HERE}/usr/bin:${HERE}/usr/bin/_internal/bin:${PATH}"
...
# argostranslate包目录配置 - 智能选择可写目录
configure_argos_packages() {
    # 如果用户已经设置了ARGOS_PACKAGES_DIR，保持不变
    if [ -n "$ARGOS_PACKAGES_DIR" ]; then
        echo "[AppRun] 使用用户指定的包目录: $ARGOS_PACKAGES_DIR" >&2
        export XDG_DATA_HOME="$HOME/.local/share"
        return
    fi
    
    USER_ARGOS_DIR="$HOME/.local/share/argos-translate/packages"
    SYS_ARGOS_DIR1="/usr/local/share/argos-translate/packages"  
    SYS_ARGOS_DIR2="/usr/share/argos-translate/packages"
    APPIMAGE_ARGOS_DIR="${HERE}/usr/bin/_internal/argos_packages"
    
    echo "[AppRun] 搜索argostranslate语言包..." >&2
    
    if [ -d "$USER_ARGOS_DIR" ] && [ "$(ls -A "$USER_ARGOS_DIR" 2>/dev/null)" ]; then
        export ARGOS_PACKAGES_DIR="$USER_ARGOS_DIR"
        export XDG_DATA_HOME="$HOME/.local/share"
        echo "[AppRun] 使用用户包目录: $USER_ARGOS_DIR ($(ls "$USER_ARGOS_DIR" | wc -l) 个包)" >&2
        return
    fi
    
    for dir in "$SYS_ARGOS_DIR1" "$SYS_ARGOS_DIR2"; do
        if [ -d "$dir" ] && [ "$(ls -A "$dir" 2>/dev/null)" ]; then
            export ARGOS_PACKAGES_DIR="$dir"
            export XDG_DATA_HOME="$HOME/.local/share"
            echo "[AppRun] 使用系统包目录: $dir ($(ls "$dir" | wc -l) 个包)" >&2
            return
        fi
    done
    
    echo "[AppRun] 未找到现有语言包，创建用户包目录..." >&2
    mkdir -p "$USER_ARGOS_DIR"
    export ARGOS_PACKAGES_DIR="$USER_ARGOS_DIR"
    export XDG_DATA_HOME="$HOME/.local/share"
    echo "[AppRun] 包目录设置为: $USER_ARGOS_DIR" >&2
    
    if [ -d "$APPIMAGE_ARGOS_DIR" ] && [ "$(ls -A "$APPIMAGE_ARGOS_DIR" 2>/dev/null)" ]; then
        echo "[AppRun] 发现AppImage内置语言包，复制到用户目录..." >&2
        cp -r "$APPIMAGE_ARGOS_DIR"/* "$USER_ARGOS_DIR/" 2>/dev/null || {
            echo "[AppRun] 内置包复制失败，但用户目录可用于新下载" >&2
        }
        builtin_count=$(ls "$APPIMAGE_ARGOS_DIR" | wc -l)
        echo "[AppRun] 已复制 $builtin_count 个内置语言包到用户目录" >&2
    fi
}

verify_package_permissions() {
    if [ -n "$ARGOS_PACKAGES_DIR" ]; then
        if [ -w "$ARGOS_PACKAGES_DIR" ]; then
            echo "[AppRun] ✅ 包目录可写: $ARGOS_PACKAGES_DIR" >&2
        else
            echo "[AppRun] ⚠️  包目录只读: $ARGOS_PACKAGES_DIR" >&2
            echo "[AppRun] 语言包下载可能失败，考虑使用 argospm 命令行工具" >&2
        fi
        
        if [ -d "$ARGOS_PACKAGES_DIR" ]; then
            package_count=$(ls -1 "$ARGOS_PACKAGES_DIR" 2>/dev/null | wc -l)
            echo "[AppRun] 当前已安装 $package_count 个语言包" >&2
        fi
    fi
}
...
main() {
    ...
    main() {
        ...
        # 验证argostranslate安装
        if ! verify_argostranslate_installation; then
            log_error "argostranslate验证失败，无法继续"
            cleanup_build_temp
            exit 1
        fi
