...
class LanguagePackDialog(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        self.offline_info = QLabel(
            "<b>注意: 当前处于离线模式</b><br>"
            "由于无法连接语言包服务器，您看到的是内置语言包列表。<br>"
            "要使用完整功能，请确保您的设备连接到互联网。<br>"
            "安装功能在离线模式下仅创建占位文件，实际翻译需要手动安装语言包。"
        )
...
class ScreenTranslator(QMainWindow):
    ...
    def __init__(self):
        ...
        self.init_global_mouse_listener()  # 初始化全局鼠标监听
        
        # 设置窗口属性
        self.setWindowTitle("Skylark Translation V1.0")
    ...
    def init_translator(self):
        """初始化翻译引擎"""
        if self.translator:
            self.update_status("正在初始化翻译引擎...")
            threading.Thread(target=self.translator.initialize, daemon=True).start()
        else:
            self.update_status("argostranslate 不可用，仅支持OCR功能")
        ...
        try:
            while not self.status_queue.empty():
                ...
                if "随时可用" in message:
                    ...
                    self.update_status("翻译引擎已就绪！双击选择框进行翻译。")
        except queue.Empty:
            pass
    ...
    def configure_languages(self):
        ...
        if not self.translator or not self.translator.available_languages:
            QMessageBox.warning(self, "警告", "语言列表未加载，请稍后重试")
            return
        ...
        if dialog.exec_() == QDialog.Accepted:
            ...
            if self.translator:
                ...
                self.update_status(f"语言设置已更新: 源语言={SOURCE_LANG}, 目标语言={TARGET_LANG}, OCR语言={ocr_lang}")
                threading.Thread(target=self.translator.initialize, daemon=True).start()
    ...
    def process_translation(self):
        ...
        try:
            ...
            # 3. 如果需要在线翻译，则启动后台线程
            if self.translator and self.translation_ready:
                self.update_ui_signal.emit("正在翻译文本...", "正在翻译...")

                def translate_and_update():
                    """这个后台线程现在负责完成任务并释放锁"""
                    try:
                        translated_text = self.translator.translate(original_text, SOURCE_LANG, TARGET_LANG)
                        self.append_translation(f"翻译: {translated_text}")
                        self.update_ui_signal.emit("翻译完成", translated_text)
                    except Exception as e:
                        import traceback
                        error_msg = f"翻译错误: {e}"
                        print(f"{error_msg}\n{traceback.format_exc()}")
                ...
                # 如果不需要在线翻译，只显示OCR结果
                self.update_ui_signal.emit("显示OCR结果", original_text)
